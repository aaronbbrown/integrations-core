...
class Couchbase(AgentCheck):
    ...
    NODE_MEMBERSHIP_METRIC = 'couchbase.by_node.cluster_membership'
    ...
    NODE_HEALTH_METRIC = 'couchbase.by_node.health_status'
    ...
    def _create_metrics(self, instance_state, data, tags=None):
        # Get storage metrics
        ...
        # Get tasks, we currently only support 'rebalance' tasks
        ...
        if rebalance_status is not None and instance_state.previous_status != rebalance_status:
            ...
            if rebalance_status == 'error':
                ...
                rebalance_event = self._create_event('error', msg_title, msg, tags)

            # We only want to fire a 'completion' of a rebalance event so we make sure we're not firing an event
            #   on first run
                ...
                rebalance_event = self._create_event('info', msg_title, msg, tags)

            # If a rebalance task is running, fire an event. This will also fire an event if a rebalance task was
            #   already running when the check runs.
            elif rebalance_status == 'running':
                # Check the message associated with the status to determine what to say in the event
                if rebalance_msg == 'rebalance':
                    msg_title = 'Rebalancing'
                    msg = 'is rebalancing.'
                elif rebalance_msg == 'gracefulFailover':
                    msg_title = 'Failing over gracefully'
                    msg = 'is failing over gracefully.'
                else:
                    msg_title = 'Unknown'
                    msg = 'has an unknown status.'
    ...
    def _process_cluster_health_data(self, node_name, node_stats, tags):
        ...
        cluster_health_tags = list(tags)
        cluster_health_tags.append(['node:{}'.format(node_name), 'device_name:{}'.format(node_name)])
    ...
    def _create_event(self, alert_type, msg_title, msg, tags=None):
        """
        Create an event object
        """
        msg_title = '{}: {}'.format(self.hostname, msg_title)
        msg = 'Couchbase: {} {}'.format(self.hostname, msg)

        return {
            'timestamp': int(time.time()),
            'event_type': 'couchbase',
            'host': self.hostname,
            'msg_text': msg,
            'msg_title': msg_title,
            'alert_type': alert_type,
            'source_type_name': self.SOURCE_TYPE_NAME,
            'event_object': self.hostname,
            'tags': tags
        }
    ...
    def check(self, instance):
        ...
        self._create_metrics(instance_state, data, tags=list(set(tags)))
        ...
        try:
            ...
            for task in tasks:
                ...
                if 'errorMessage' in task:
                    ...
                    couchbase['tasks'][task_type] = (task['status'], task['subtype'])
